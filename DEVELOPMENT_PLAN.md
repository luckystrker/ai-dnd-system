...
Кампании, участники и листы персонажей живут в SQLite
(`agent/lib/campaigns/store-sqlite.ts`, интерфейс `CampaignStore`; откат на
MD-стор — `CAMPAIGN_STORE=markdown`, перенос данных — `npm run migrate:sqlite`).
...
5. **M5 (готово)**: безопасность и баги по итогам внешнего аудита: привязка
   действий к персонажу владельца (`skill_check`/`combat` только для владельца
   или DM), валидация `attacker` против партии, `enemy` обязателен только для
   атаки, rate limiting и лимит размера payload на webhook, явное вступление
   `/join` вместо авто-вступления, защита slug от path traversal, осмысленные
   окна контекста по длине кампании (64K/256K/900K), фикс рассинхрона
   `message_thread_id` в контексте канала; хранилище кампаний переведено на
   SQLite (MD-стор сохранён как откат и источник миграции).
6. **M6 (готово)**: квесты и открытые нити как first-class сущности в SQLite
   (таблицы `quests`, `open_threads` в сторе, откат на MD-стор); тулы
   `create_quest` / `update_quest` / `complete_quest` / `list_quests` /
   `append_thread` / `resolve_thread` / `list_open_threads`; награды по
   таблицам сложности и уровня партии (`lib/rewards.ts`: XP, золото,
   свободная часть в rewardPlan.note); поток level-up (`level_up`: hit die +
   ВЫН, новая способность); секции «Активные квесты» и «Открытые нити» в
   инъекции памяти (без капа, в отличие от key-events); chronicler закрывает
   день с сверкой квестов/нитей и выдачей наград; команда `/quests` для
   игроков; рекап на старте сессии, дедлайны квестов и признаки живого мира
   (правила в instructions.md).
7. **Далее**: адаптер хранилища для деплоя (Vercel: эфемерная ФС — нужен
   S3/KV/Postgres за интерфейсами сторов для транскриптов/NPC); расширение
   правил по мере необходимости; hard-guardrail против игры DM за чужих PC
   (отложенный A3 из спеки групповой игры).
